...
class AutogenChat:
    ...
    def __init__(self, chat_id=None, websocket=None, agents=[]):
        ...
        for agent in agents:
            self.agents.append(self.get_agent_from_dict(agent))

        self.agents = [self.creator, self.critic, self.coder, self.qa]
        self.user_proxies = [self.user_proxy]

        self.groupchat = autogen.GroupChat(
            agents=self.user_proxies + self.agents,
            messages=[],
            max_round=20,
